...
import { writeCross, setDirectionIsDown, switchSeparation, setQuestion,
  ensureQuestion, setNewsName, ensureNewsName, setSubtitle, ensureSubtitle, setDate,
  setAbbreviation, ensureAbbreviation, reset } from "./actions"

class App extends React.Component {
  constructor(props) {
    super(props);
    this.input_refs = new Array(11).fill(null).map(x => new Array(12).fill(null).map(y => React.createRef()))
  }
  ...
  render() {
    ...
    const right_coords = right.flatMap((val, x) => val.map((innerVal, y) => innerVal ? List([x, y]) : undefined)).filter(x => x)
    const down_coords = down.flatMap((val, y) => val.map((innerVal, x) => innerVal ? List([x, y]) : undefined)).filter(x => x)
    const numbers = new Map(right_coords.concat(down_coords).toSet().toList().sortBy(sortCoords).map((cell, i) => [cell, i+1]))
        ...
        <div style={{display: "inline-block"}}>
          ...
          <p className="no-print">
            <div style={{float: "left"}}>
              ...
              Rüber <Switch checked={writingDown} style={{background: "rgba(0, 0, 0, 0.25)"}} onChange={handleDirectionChange}/> Runter
            </div>
            <div style={{float: "right"}}>
              <Popconfirm
          ...
          <div style={{
            ...
            {right_coords.sortBy(sortCoords).map(key => {
              ...
              const word = chars.get(row_i).slice(col_i).takeWhile(
                (v, k) => k === 0 || ! right.getIn([row_i, col_i+k])
              ).reduce(
                (prev, el, i) => el ? prev + el : prev + "_",
                ""
              )
              return (<div>
                {number} <i className="no-print">{word}</i> <ContentEditable
                  tagName="span"
                  html={question}
                  onChange={e => handleSetQuestion(false, row_i, col_i, e.target.value)}
            ...
            {down_coords.sortBy(sortCoords).map(key => {
              ...
              const chars_transposed = chars.get(0).zip(...chars.rest()).map(List)
              const word = chars_transposed.get(col_i).slice(row_i).takeWhile(
                (v, k) => k === 0 || ! down.getIn([col_i, row_i+k])
              ).reduce(
                (prev, el, i) => el ? prev + el : prev + "_",
                ""
              )
              return (<div>
                {number} <i className="no-print">{word}</i> <ContentEditable
                  tagName="span"
                  html={question}
                  onChange={e => handleSetQuestion(true, row_i, col_i, e.target.value)}
...
const mapDispatchToProps = dispatch => ({
  handleCellChange: (row_i, col_i, chars) => dispatch(writeCross(row_i, col_i, chars)),
  handleDirectionChange: (isDown) => dispatch(setDirectionIsDown(isDown)),
  handleSwitchSeparation: (row_i, col_i) => dispatch(switchSeparation(row_i, col_i)),
  handleSetQuestion: (isDown, row_i, col_i, question) => dispatch(setQuestion(isDown, row_i, col_i, question)),
  handleEnsureQuestion: (isDown, row_i, col_i) => dispatch(ensureQuestion(isDown, row_i, col_i)),
